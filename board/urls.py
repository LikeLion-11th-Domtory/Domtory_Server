...
urlpatterns = [
    # 게시글 API
    path('post/create/<int:board_id>/', PostCreateViewV2.as_view()), # 게시글 생성
    ...
    path('post/list/<int:board_id>/', PostListByDormView.as_view()), # 기숙사별 특정 게시판의 게시글 리스트
    path('post/latest/<int:board_id>/', RecentPostListView.as_view()), # 기숙사별 특정 게시판의 최근 5개 게시글
    path('post/search/<int:board_id>/', PostSearchViewV2.as_view()), # 게시글 검색
